    ...
    try {
        ...
        const updateStatus = async () => {
            try {
                ...
                const power = targetDevice.param.present.power;
                const mode = targetDevice.param.present.mode;
                const temperature = targetDevice.param.present.temperature;
                if (dataElement) {
                    dataElement.textContent = `${deviceInfo.status.power.name}:${deviceInfo.status.power[power]}, ${deviceInfo.status.mode.name}:${deviceInfo.status.mode[mode]}, ${deviceInfo.status.temperature.name}:${temperature} ${deviceInfo.status.temperature.unit}`;
                }
            } catch (err) {
